...
//        if (!wishlist.getBooks().stream()
//                .filter(i -> i.getId()==bookId).toList().isEmpty())
//            throw new RuntimeException("Book already in wishlist!");
//        wishlist.getBooks().add(book);
//        return Optional.of(wishlistRepository.save(wishlist));
        if (getWishlist(userId).isPresent()) {
            Wishlist wishlist = getWishlist(userId).get();
            Book book = bookRepository.findById(bookId)
                    .orElseThrow(()-> new RuntimeException("Book not found"));
            if (book.getAvailableCopies() <= 0){
                throw new NotAvailableCopiesOfBook();
            }
            if (!wishlist.getBooks().stream().filter(i->i.getId()==bookId).toList().isEmpty())
                throw new RuntimeException("Book already in wishlist.");
            wishlist.getBooks().add(book);
            return Optional.of(wishlistRepository.save(wishlist));
        }
    ...
    @Override
    public Optional<Wishlist> getWishlist(String username) {
        User user=userRepository.findByUsername(username).orElseThrow(() -> new UsernameNotFoundException(username));
        return Optional.of(wishlistRepository.findByUser(user).orElseGet(()->
